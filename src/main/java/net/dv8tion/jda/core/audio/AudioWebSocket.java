...
public class AudioWebSocket extends WebSocketAdapter
{
    ...
    {
        LOG.error("Callback Error for AudioWebSocket", cause);
    }
    ...
    {
        if (keepAliveRunnable != null)
            LOG.error(api.getShardMarker(), "Setting up a KeepAlive runnable while the previous one seems to still be active!!");
